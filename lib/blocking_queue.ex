defmodule BlockingQueue do
  ...
  def start_link(n, options \\ []) do
    GenServer.start_link(__MODULE__, n, options)
  end
  ...
  # start a list of waiting pushers when the first client tries to push to a
  # full queue
  def handle_call(
    {:push, item},
    from,
    {max, queue = {left, right}}
  ) when length(left) + length(right) >= max do
    {:reply, :block, {max, queue, :push, [{from, item}]}}
  end

  # prepend new waiter to list of waiting pushers when they try to push to a
  # full queue
  def handle_call(
    {:push, item},
    from,
    {max, queue = {left, right}, :push, [next|rest]}
  ) when length(left) + length(right) >= max do
  ...
  # start a list of waiting poppers when the first client tries to pop from
  # the empty queue
  def handle_call(:pop, from, {max, @empty_queue}) do
    {:reply, :block, {max, @empty_queue, :pop, [from]}}
  end

  # prepend new waiter to list of waiting poppers when they try to pop from
  # an empty queue
  ...
  # accept an item pushed by the last in a list of waiting pushers (taking
  # last makes this FIFO)
  def handle_call(
    :pop,
    _,
    {max, queue, :push, waiters}
  ...
  def handle_call(
    ...
    {still_waiters, filtered_waiters} =
      Enum.partition waiters, &f.(elem(&1, 1))

    Enum.each filtered_waiters, &send(elem(elem(&1, 0), 0), :awaken)

    {rest, next} =
      Enum.split still_waiters, :queue.len(filtered_queue) - max

    final_queue =
      Enum.reduce(Enum.reverse(next), filtered_queue, fn({next, item}, q) ->
      send(elem(next, 0), :awaken)
      :queue.in(item, q)
    end)

    {
      ...
      nil,
      (
        if Enum.empty?(rest), do: {max, final_queue},
        else: {max, final_queue, :push, rest}
      )
    }
  end
  ...
  `timeout` (optional) is the timeout value passed to GenServer.call
  (does not impact how long pop will wait for a message from the queue)
  """
  ...
  @doc """
  Filters the queue by removing all items for which the function `func`
  returns false.
